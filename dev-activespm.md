...
- 2026-09-13: Implemented the polling MMIO control register block with atomic descriptor capture, stable DMA request handshaking, live and final progress, sticky W1C completion status, busy-command rejection, and end-to-end MMIO-driven DMA tests.
- 2026-09-13: Added a dual-Rocket, dual-Gemmini, dual-ActiveSPM 3x2 Mesh NoC integration config, exact endpoint-mapping checks, a public polling C header, and a two-hart bare-metal data-path test owned by the ActiveSPM repository.
- 2026-09-13: Removed the DMA TileLink D-to-A combinational error dependency, added non-flow-through A/D register boundaries to both DMA clients, and extended error/drain coverage across the external and local ports.
